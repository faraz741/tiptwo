...
@Component({
  selector: 'app-the-system',
  templateUrl: './the-system.component.html',
  styleUrls: ['./the-system.component.css']
})
export class TheSystemComponent {
  ...
  submitEditorContent(){
    ...
    this.appService.post(API_URL.update_the_system, bodyPayload).subscribe({
      next: (res: any) => {
        console.log(res)
        // this.loader = false
        if (res.success == true) {
    ...
    this.appService.get(API_URL.getHowitWorks).subscribe({
      next: (res: any) => {
        ...
        if (res.success == true) {
          ...
          this.data = res.data[0].the_system
        } else {
